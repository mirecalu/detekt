...
Usage: detekt [options]
  Options:
    ...
    --build-upon-default-config
      ...
      Default: false
    --classpath, -cp
      EXPERIMENTAL: Paths where to find user class files
    --config, -c
    ...
    --fail-fast
      Shortcut for 'build-upon-default-config' together with all available
      rules active and exit code 0 only when no code smells are found.
      Additional configuration files can override properties but not the
      'active' one.
      Default: false
    --generate-config, -gc
